...
for n in 1:10
    ...
    for_x = Array{Int64,1}(undef, numSamples)
    fill!(for_x, n)
    append!(x, for_x)
    append!(y, checkH.times)
end
scatter([x], [y], title = "Autoplotted Graph Vertices", xlabel = "Graph Dimensions (Path Graph Vertex count)", ylabel = "Single Hom Calculation Time (ns)")
